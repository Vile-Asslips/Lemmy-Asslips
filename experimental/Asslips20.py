#!/usr/bin/env python3
"""
Asslips19.py – Lemmy GPT bots enhanced to force inclusion of title/body words
  • adaptive token budgeting
  • externalized, diversified fallback templates (expanded)
  • direct grounding: title/body or comment in prompt (mandatory verbatim mention)
  • dynamic few-shot + keyword grounding
  • forced injection of title/body words into topics
...
class FallbackManager:
    ...
    def choose(self, topics: list[str], post_id: Any) -> str:
        ...
        candidates = [i for i in all_indices if i not in used] or all_indices
        idx = random.choice(candidates)
        template = self.templates[idx]
        filled = template.replace("{topic}", topic).replace("{related}", related)
        self.recent.setdefault(post_id, deque(maxlen=self.history_size)).append(idx)
        return filled
...
def clean(text: str) -> str:
    ...
    # remove any <|...|> style control sequences
    text = re.sub(r"<\|.*?\|>", "", text)
    ...
    # remove stray angle-bracket tokens
...
def extract_keywords(text: str, max_words: int = 5) -> list[str]:
    ...
    return sorted(freq, key=lambda k:(-freq[k], len(k)))[:max_words]
...
class BotThread(threading.Thread):
    def __init__(self, bot_cfg: MappingProxyType, global_cfg: MappingProxyType, fallback_manager: FallbackManager):
        ...
        self.roll_needed = int(bot_cfg.get("comment_roll", 70))
    ...
    def _is_incomplete_reply(self, text: str) -> bool:
        ...
        if s.endswith(('.', '!', '?', '...')):
            return False
        lw = s.split()[-1].lower()
        if lw in {'and','or','but','with','to','for','because','so','if','that','which','when','while','as','than','then'}:
            return True
        if len(s.split()) < 8:
            return True
        return False
    ...
    def _gen(self, prompt: str, skip_toxic: bool = False, temperature: float | None = None,
        ...
        # unchanged from Asslips18 for brevity
    ...
    def _try_complete(self, candidate: str, topic_str: str, temp: float, tp: float) -> str:
        # unchanged aggressive completion from Asslips18
        """
    ...
    def _attempt_replies(self, sources: list[dict[str, Any]]) -> None:
        ...
        for src in sources:
            ...
            # === NEW: forced injection of title/body keywords ===
            ...
            if is_comment:
                prompt = (
                    ...
                    "Instructions:\n"
                    "1. Briefly acknowledge something specific from that comment and include at least one of those words verbatim.\n"
                ...
                prompt = (
                    ...
                    f"Title: \"{title}\"\n"
                    f"Body: \"{body[:1000].rstrip()}...\"\n\n"
                    f"Main topics: {topic_str}\n"
                    "Instructions:\n"
                    "1. Briefly acknowledge something specific from the title or body and include at least one of those words verbatim.\n"
                    ...
                    "4. Keep it coherent, avoid unrelated rambling, and end with a complete thought. Limit to 2-3 sentences. Reply:"
                )
            ...
            for attempt in range(3):
                ...
                # === NEW: enforce mention of at least one forced keyword ===
                if not any(re.search(rf"\b{re.escape(w)}\b", candidate, re.IGNORECASE)
